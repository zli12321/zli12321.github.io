---
title: "SciDoc2Diagrammer-MAF: Towards Generation of Scientific Diagrams from Documents guided by Multi-Aspect Feedback Refinement"
collection: publications
category: conferences
permalink: /publication/2024-11-11-scidoc2diagrammer
authors: 'Ishani Mondal, Zongxia Li, et al.'
excerpt: 'Generates scientific diagrams from documents using multi-aspect feedback refinement.'
date: 2024-11-11
venue: 'EMNLP Findings 2024'
citation: 'Ishani Mondal, Zongxia Li, et al. (2024). "SciDoc2Diagrammer-MAF: Towards Generation of Scientific Diagrams from Documents guided by Multi-Aspect Feedback Refinement." <i>Findings of EMNLP</i>.'
header:
  teaser: diagrammer.png
---

SciDoc2Diagrammer-MAF generates scientific diagrams from documents using multi-aspect feedback refinement.
